  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const invoicesHTML = allShipments.map((s, i) => {
        ...
        const probText = s.problem || s.lead_id?.problem || '';
        const rows = data.allLines.map(li => `<tr>
          <td style="${c}text-align:center;border-left:none;">${li.sno}</td>
          <td style="${c}font-weight:600;">
            ${li.description}
            ${(!li.isDoctor && probText) ? `<div style="font-size:10px;color:#444;font-style:italic;margin-top:2px;font-weight:500;">Problem: ${probText}</div>` : ''}
          </td>
        ...
        <div style="display:flex;border:1px solid #000;border-top:none;">
          ...
          <div style="flex:1;padding:12px 14px;">
            ...
            <div style="font-size:11px;color:#333;margin-top:3px;line-height:1.4;">${custAddr}<br/>${s.billing_phone ? 'Phone: ' + s.billing_phone + '<br/>' : ''}${probText ? '<div style="margin-top:3px;font-weight:700;color:#c2410c;">Problem: ' + probText + '</div>' : ''}</div>
          </div>
        </div>

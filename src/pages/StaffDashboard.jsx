...
export default function StaffDashboard() {
  ...
  useEffect(() => {
    ...
    fetchMyDeliveryStats(deliveryMonth.month, deliveryMonth.year)
      .then(d => { if (!cancelled) setMyDelivery(d); })
      .catch(() => {})
      .finally(() => { if (!cancelled) setDeliveryLoading(false); });
    return () => { cancelled = true; };
  }, [deliveryMonth, user?.role]);
  ...
  const MOTIVATIONS = [
    ...
    'Thursday THUNDER — Go harder than yesterday!',
  ...
  const DEPT_COLOR = {
    ...
    ortho: 'bg-green-50 text-green-700 border-green-100',
    skin: 'bg-pink-50 text-pink-700 border-pink-100',
    ...
    <div className="space-y-6 pb-10">
      ...
      {streak >= 3 && (
        <div className="rounded-2xl px-5 py-3 flex items-center gap-3" style={{ background: 'linear-gradient(135deg, #78350f, #92400e)', border: '1px solid #b45309' }}>
          <svg className="w-5 h-5 text-amber-400" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M12.395 2.553a1 1 0 00-1.45-.385c-.345.23-.614.558-.822.88-.431.67-.803 1.492-1.127 2.45-.675 1.997-1.107 4.544-1.107 7.002 0 3.314 2.462 6 5.5 6s5.5-2.686 5.5-6c0-2.828-1.517-5.59-3.238-7.556a12.825 12.825 0 00-3.256-2.391z" clipRule="evenodd"/></svg>
          <div>

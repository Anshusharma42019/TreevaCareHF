...
const inp = 'w-full border-2 border-slate-200 rounded-xl px-3.5 py-2.5 text-sm font-bold text-slate-900 focus:outline-none focus:border-emerald-600 focus:ring-1 focus:ring-emerald-600 bg-white placeholder-slate-400 shadow-sm';
const Field = ({ label, children }) => (
  <div>
    <label className="block text-xs font-black text-slate-700 mb-1.5 uppercase tracking-wide">{label}</label>
    {children}
  </div>
);

const MEDICINES = [
  { name: 'Male Wellness', sku: 'MALE-001' },
  { name: 'Ortho Wellness', sku: 'ORTHO-001' },
  { name: 'Skin Wellness', sku: 'SKIN-001' },
  { name: 'Ayurvedic Oil', sku: 'AYU-001' },
  { name: 'Treeva Wellness Kit', sku: 'TWK-001' },
...
export default function Shiprocket({ initialSection, initialReturnsTab = 'returns' }) {
  ...
  const [order, setOrder] = useState({
    ...
    order_items: [{ name: 'Male Wellness', sku: getMedicineSku('Male Wellness'), units: 1, selling_price: '' }],
    payment_method: 'COD',
  ...
  const fetchAddress = async (pin, type) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'https://xluxtreeva.de/api/v1'}/pincode/${pin}`);
  ...
  const renderStep = () => {
    if (step === 0) return (
      <div style={{ background: '#ffffff', borderRadius: 16, padding: '24px 28px', border: '1.5px solid rgba(0,0,0,0.1)', boxShadow: '0 4px 16px rgba(0,0,0,0.04)' }} className="space-y-5 max-w-xl">
        <div className="flex items-center gap-3 pb-3 border-b border-slate-100">
          <div className="w-10 h-10 rounded-xl bg-emerald-500/15 border border-emerald-500/30 text-emerald-700 flex items-center justify-center font-bold">
          ...
          <div>
            <h4 className="text-base font-black text-slate-900 tracking-tight">Shiprocket API Authentication & Token Management</h4>
            <p className="text-xs font-bold text-slate-500 mt-0.5">Generates secure API session token from server credentials to execute logistics tasks</p>
          </div>
        </div>

        {token ? (
          <div className="space-y-4">
            <div className="flex items-center gap-3.5 bg-emerald-50 border-2 border-emerald-200 rounded-xl p-4">
              <div className="w-9 h-9 rounded-full bg-emerald-600 text-white flex items-center justify-center font-bold text-sm shrink-0 shadow-sm">✓</div>
              <div>
                <span className="text-emerald-950 text-sm font-black tracking-tight">Active API Token Connected</span>
                <p className="text-emerald-700 text-xs font-bold mt-0.5">Expires: {new Date(Number(localStorage.getItem(TOKEN_EXP_KEY))).toLocaleString()}</p>
              </div>
            </div>
            <div className="flex flex-wrap gap-3 pt-1">
              <button onClick={() => call(async () => {
                ...
                return res.data;
              })} className="px-5 py-2.5 rounded-xl font-extrabold text-xs text-slate-700 bg-slate-100 hover:bg-slate-200 border border-slate-300 transition-all">
                Refresh Token
              </button>
              <button onClick={() => goStep(1)} className="px-5 py-2.5 rounded-xl font-extrabold text-xs text-white shadow-md hover:shadow-lg transition-all" style={{ background: 'linear-gradient(135deg, #16a34a, #15803d)' }}>
                Proceed to Step 2: Serviceability →
              </button>
              <button onClick={() => goStep(2)} className="px-5 py-2.5 rounded-xl font-extrabold text-xs text-white shadow-md hover:shadow-lg transition-all" style={{ background: 'linear-gradient(135deg, #2563eb, #1d4ed8)' }}>
                Proceed to Step 3: Create Order →
              </button>
            </div>
          </div>
        ) : (
          <div className="space-y-4">
            <div className="bg-amber-50 border-2 border-amber-200 rounded-xl p-4">
              <span className="text-amber-950 text-sm font-black">No Active Token Found</span>
              <p className="text-amber-800 text-xs font-bold mt-1">Click the button below to generate a new 24-hour API token using server credentials.</p>
            </div>
            <button onClick={() => call(async () => {
              const res = await srSvc.login();
              saveToken(res.data.data.token);
              setToken(res.data.data.token);
              return res.data;
            })} className="px-6 py-3 rounded-xl font-extrabold text-sm text-white shadow-lg hover:shadow-xl transition-all flex items-center gap-2" style={{ background: 'linear-gradient(135deg, #16a34a, #15803d)' }}>
                ...
                return (
                  <div key={r._id} className="px-5 py-3 flex items-center gap-3 hover:bg-gray-50/50 transition-colors">
                    ...
                    <button onClick={() => {
                      ...
                      let productName = r.title || r.task?.title || 'Male Wellness';
                      const customerName = (r.lead?.name || '').trim();
                      const dept = (r.department || r.lead?.department || '').toLowerCase();
                      if (productName.toLowerCase().startsWith('call ') || productName.trim() === customerName) {
                        if (dept === 'ortho') productName = 'Ortho Wellness';
                        else if (dept === 'skin') productName = 'Skin Wellness';
                        else productName = 'Male Wellness';
                      }
                      const price = r.price || '';
                      setOrder(p => ({
                        ...p,
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Step Content */}
      <div className="bg-white rounded-2xl border-2 border-slate-200/80 p-6 shadow-sm space-y-5">
        <div className="flex items-center justify-between pb-3 border-b border-slate-100">
          <div className="flex items-center gap-3">
            <span className="w-8 h-8 rounded-xl bg-green-600 text-white text-sm font-black flex items-center justify-center shadow-sm">
              {step + 1}
            </span>
            <div>
              <h3 className="font-black text-slate-900 text-base tracking-tight">{STEPS[step]}</h3>
              <p className="text-xs font-bold text-slate-500">Step {step + 1} of 10 · Shiprocket Logistics Workflow</p>
            </div>
          </div>
          {token ? (
            <span className="text-[11px] font-extrabold px-3 py-1 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 flex items-center gap-1.5">

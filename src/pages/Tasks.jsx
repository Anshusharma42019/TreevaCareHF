...
export default function Tasks() {
  ...
  const handlePincodeChange = async (val) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'https://xluxtreeva.de/api/v1'}/pincode/${val}`);
        ...
        <div className="hidden lg:flex flex-col w-[60%] bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden h-full">
          ...
          <div className="px-5 py-4 overflow-y-auto flex-1 custom-scrollbar">
            ...
            <div className="mt-8">
              ...
              {selected.status === 'cnp' && (
                <div className="flex items-center justify-center gap-1.5 bg-orange-50 text-orange-600 rounded-xl py-3 mt-4 text-xs font-bold">
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path d="M20 6L9 17l-5-5"/></svg>
                  Marked as CNP
                </div>
              )}
              {selected.status === 'cancel_call' && selected.lead?.status === 'follow_up' && (
                <div className="flex items-center justify-center gap-1.5 bg-amber-50 text-amber-600 rounded-xl py-3 mt-4 text-xs font-bold">
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path d="M20 6L9 17l-5-5"/></svg>
                  Moved to Call Again
                </div>
              )}
              {selected.status === 'cancel_call' && selected.lead?.status !== 'follow_up' && (
                <div className="flex items-center justify-center gap-1.5 bg-red-50 text-red-600 rounded-xl py-3 mt-4 text-xs font-bold">
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path d="M20 6L9 17l-5-5"/></svg>
                  Marked as Loss
                </div>
              )}
          ...
          <div className="px-5 py-4 border-t border-gray-50 bg-white shrink-0">
            <div className="grid grid-cols-2 gap-2">
              ...
              {selected.status === 'completed' && (
                <div className="flex items-center justify-center gap-1.5 bg-emerald-50 text-emerald-600 rounded-xl py-3 text-xs font-bold">
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path d="M20 6L9 17l-5-5"/></svg>
                  Task Finished
                </div>
              )}
          ...
          <Modal hideHeader={true} onClose={() => setSelected(null)}>
            <div className="flex flex-col h-[80vh]">
              ...
              <div className="flex-1 overflow-y-auto px-1 space-y-0 custom-scrollbar">
                 ...
                 <DetailRow label="Due Date" value={new Date(selected.dueDate).toLocaleString()} />
                 <DetailRow label="Status" value={selected.status} color={selected.status === 'completed' ? 'green' : 'amber'} />
                 <DetailRow label="Department" value={selected.department ? selected.department.toUpperCase() : null} />
              ...
              <div className="pt-4 pb-2 flex flex-col gap-2">
                ...
                {selected.status === 'cnp' && (
                  <div className="w-full py-3 bg-orange-50 text-orange-600 rounded-xl text-sm font-bold text-center flex items-center justify-center gap-1.5">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path d="M20 6L9 17l-5-5"/></svg>
                    Marked as CNP
                  </div>
                )}
                {selected.status === 'cancel_call' && selected.lead?.status === 'follow_up' && (
                  <div className="w-full py-3 bg-amber-50 text-amber-600 rounded-xl text-sm font-bold text-center flex items-center justify-center gap-1.5">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path d="M20 6L9 17l-5-5"/></svg>
                    Moved to Call Again
                  </div>
                )}
                {selected.status === 'cancel_call' && selected.lead?.status !== 'follow_up' && (
                  <div className="w-full py-3 bg-red-50 text-red-600 rounded-xl text-sm font-bold text-center flex items-center justify-center gap-1.5">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path d="M20 6L9 17l-5-5"/></svg>
                    Marked as Loss
                  </div>
                )}

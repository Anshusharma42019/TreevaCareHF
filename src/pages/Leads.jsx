...
const DetailRow = ({ label, value }) => {
  ...
  return (
    <div className="flex items-start gap-3 py-2 border-b border-gray-50 last:border-0">
      ...
      <span className={`text-sm text-gray-800 font-medium flex-1 whitespace-pre-wrap leading-relaxed ${isMultiLine ? '' : 'capitalize'}`}>{value}</span>
    </div>
  );
};
...
export default function Leads() {
  ...
  const handleDistributeAbsent = async () => {
    if (!window.confirm('Redistribute all leads of ABSENT sales staff to active staff?')) return;
    setLoading(true);
    try {
      const res = await distributeAbsentSalesLeads();
      toastSuccess(res.message, 'Redistributed');
      await load();
    } catch (err) {
      toastError(err.response?.data?.message || 'Failed to redistribute absent leads', 'Error');
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-4">
            ...
            {canManage && (
              <>
                ...
                <button onClick={handleDistributeAbsent} disabled={loading}
                  className="w-full md:w-auto px-5 py-2.5 rounded-xl text-[10px] font-black text-white shadow-lg hover:shadow-orange-200 transition-all flex items-center justify-center gap-2 shrink-0 active:scale-95 disabled:opacity-50"
                  style={{ background: 'linear-gradient(135deg, #f97316, #ea580c)' }}>
                  {t('REDISTRIBUTE ABSENT')}
                </button>
              </>
            )}
          </div>
            ...
            <form onSubmit={handleUpdate} className="space-y-5">
              ...
              <div className="grid grid-cols-3 gap-2 pt-2">
                ...
                <button type="button" disabled={loading} onClick={() => {
                  ...
                  setData(d => ({ ...d, leads: (d.leads || []).filter(l => l._id !== targetId) }));
                  setSelected(null); setChatNotes([]);
                  markCNP(targetId).then(() => load(true)).catch(() => load(true));
                ...
                <button type="button" disabled={loading} onClick={() => {
                  ...
                  setData(d => ({ ...d, leads: (d.leads || []).filter(l => l._id !== targetId) }));
                  setSelected(null); setChatNotes([]);
                  createCallAgain(targetId).then(() => load(true)).catch(() => load(true));

    ...
    <div className="max-w-4xl mx-auto space-y-4 pb-10">
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          onClick={async () => {
            try {
              const res = await smxSvc.getInvoice(order.order_id);
              if (res.data && res.data.size > 0) {
                const blob = new Blob([res.data], { type: 'application/pdf' });
                window.open(URL.createObjectURL(blob), '_blank');
              } else alert('Invoice not available yet.');
            } catch (e) { alert('Error fetching invoice: ' + (e.message || 'Failed')); }
          }}
          className="px-3.5 py-1.5 rounded-xl bg-orange-500 hover:bg-orange-600 text-white text-xs font-bold transition shadow-sm flex items-center gap-1">

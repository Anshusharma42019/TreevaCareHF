...
export default function Layout() {
  ...
  const DEPT_COLOR = {
    ...
    ortho: 'bg-green-50 text-green-600 border-green-100',
    skin: 'bg-pink-50 text-pink-600 border-pink-100',
    logistics: 'bg-amber-50 text-amber-600 border-amber-100',
  };
  ...
  return (
    <div className="flex h-screen overflow-hidden theme-root relative" style={{ background: 'var(--theme-bg, #f0f4f0)' }}>
      ...
      <div className="flex-1 md:ml-64 flex flex-col h-screen min-w-0 overflow-hidden">
        ...
        <header className="sticky top-0 z-10 px-4 py-3 flex items-center justify-between glass shadow-sm shadow-black/5"
          ...
          <div className="flex items-center gap-2 md:hidden">
            <span className="text-sm font-extrabold tracking-tight text-emerald-900">Treeva Homeopathy CRM</span>
          </div>
          ...
          <div className={`${mobileSearchOpen ? 'flex absolute inset-x-0 top-0 h-full bg-white z-20 items-center px-4 gap-2 animate-slide-up' : 'hidden'} md:flex md:static md:bg-transparent md:h-auto md:px-0 md:flex-1 md:items-center md:gap-4`}>
            ...
            {/* Global Phone Search */}
            <div className="relative flex-1 md:ml-4 flex items-center w-full md:w-auto">
              <UniversalSearch />
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            {/* Quick Create Prescription Button */}
            {['admin', 'manager', 'doctor'].includes(user?.role) && (
              <button
                onClick={() => setPrescriptionModalOpen(true)}
                className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-sky-600 hover:bg-sky-700 text-white text-xs font-bold shadow-sm transition-all"
            ...
            <button onClick={() => navigate('/notifications')}
              className="relative w-10 h-10 flex items-center justify-center rounded-full bg-white/75 backdrop-blur-md border border-white/80 shadow-sm hover:shadow-md transition-all text-gray-700 hover:text-emerald-700">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
            ...
            {/* Avatar Profile Pill button */}
            <div className="relative flex items-center">
              <button
                id="avatar-btn"
                onClick={() => { setAvatarOpen(p => !p); setAvatarError(''); }}
                className="flex items-center gap-2.5 px-2.5 py-1.5 rounded-full bg-white/75 backdrop-blur-md border border-white/80 shadow-sm hover:shadow-md hover:bg-white/90 transition-all text-left"
              >
                <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white shadow-sm overflow-hidden flex-shrink-0"
                  style={user?.avatar ? {} : { background: 'linear-gradient(135deg, #166534, #16a34a)' }}>
                  {user?.avatar
                ...
                <div className="hidden sm:flex flex-col pr-1">
                  <span className="text-xs font-bold text-gray-800 leading-none">{user?.name || 'Dr. Anshu Sharma'}</span>
                  <span className="text-[10px] font-semibold text-gray-500 capitalize leading-none mt-0.5">{user?.role === 'admin' ? 'Administrator' : (user?.role || 'Doctor')}</span>
                </div>
                <svg className="w-3.5 h-3.5 text-gray-500 transition-transform" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24">
                    ...
                    <div className="p-4 space-y-3">
                      ...
                      {['sales', 'support', 'logistics'].includes(user?.role) && commissionStats && (
                        <div className="flex items-center justify-between rounded-2xl px-3.5 py-2.5 border border-emerald-100"
                          ...
                          <div className="w-9 h-9 rounded-xl bg-emerald-500 flex items-center justify-center text-white shadow-md shadow-emerald-200">
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                          ...
                          <div className="flex items-center gap-2">
                            <span className="w-6 h-6 rounded-lg bg-emerald-50 text-emerald-600 flex items-center justify-center">
                              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M4.098 19.902a3.75 3.75 0 005.304 0l6.401-6.402M6.75 21A3.75 3.75 0 013 17.25V4.125C3 3.504 3.504 3 4.125 3h5.25c.621 0 1.125.504 1.125 1.125v4.072M6.75 21a3.75 3.75 0 003.75-3.75V8.197M6.75 21h13.125c.621 0 1.125-.504 1.125-1.125v-5.25c0-.621-.504-1.125-1.125-1.125h-4.072M10.5 8.197l6.401-6.402a3.75 3.75 0 015.304 0l.375.375a3.75 3.75 0 010 5.304l-6.402 6.401M10.5 8.197H3.75" />
                              </svg>
                            </span>

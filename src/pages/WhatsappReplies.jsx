                      ...
                      return (
                        <div key={i} className={`flex flex-col p-4 rounded-2xl border transition-all ${fu.completed ? 'bg-gray-50 border-gray-100 opacity-70' : isCurrent ? 'bg-emerald-50 border-emerald-200 shadow-sm ring-2 ring-emerald-500/10' : 'bg-white border-gray-100'}`}>
                          <div className="flex items-center justify-between mb-3">
                            ...
                            {fu.completed && <span className="w-5 h-5 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center text-[10px] font-bold">
                              <svg className="w-3 h-3 text-emerald-600" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path d="M20 6L9 17l-5-5"/></svg>
                            </span>}
                          </div>

...
const isExcludedProduct = (o) => {
  ...
  const prodStr = (
    prodItems.map(p => `${p.name || ''} ${p.sku || ''}`).join(' ') + ' ' +
    (o.medicine || '') + ' ' +
    (o.title || '') + ' ' +
    (o.problem || '') + ' ' +
    JSON.stringify(o.raw_response || '')
  ).toLowerCase();
...
const getShipmaxxDept = (o) => {
  ...
  if (/\b(ortho|joint|bone|knee|arthritis|pain|spine|back|joint-pain|orthopaedic)\b/i.test(prodStr)) return 'ortho';
  if (/\b(skin|acne|face|glow|derma|derma-glow|eczema|psoriasis|rash|pigmentation|cream|soap)\b/i.test(prodStr)) return 'skin';
...
const openManifest = async (awb, setActionId) => {
  ...
  try {
    ...
    if (res.data && res.data.size > 0) {
      const fullText = await res.data.text();
      let isPdf = fullText.startsWith('%PDF-');
      let isHtml = fullText.toLowerCase().includes('<html') || fullText.toLowerCase().includes('<table') || fullText.toLowerCase().includes('<body') || fullText.toLowerCase().startsWith('<!doctype');

      if (!isPdf && !isHtml) {
        try {
          const parsedJson = JSON.parse(fullText);
          throw new Error(parsedJson.message || parsedJson.detail || 'Manifest not available yet.');
        } catch (e) {
...
const openInvoice = async (order_id, setActionId, orderObj = null) => {
  if (!order_id) return alert('Order ID is required');
  if (setActionId) setActionId(`invoice-${order_id}`);
  try {
    let o = orderObj;
    if (!o) {
      try {
        const res = await smxSvc.getOrder(order_id);
        o = res.data?.data || res.data;
      } catch (err) {}
    }
    ...
    if (o) {
      const probText = o.problem || o.lead_id?.problem || '';
      const items = o.order_items || o.products || [];
      const itemName = items[0]?.name || o.medicine || o.title || 'Male Wellness';
      const subTotal = Number(o.sub_total || 1500).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
      const orderDateStr = o.createdAt ? new Date(o.createdAt).toLocaleDateString('en-IN') : new Date().toLocaleDateString('en-IN');
      const invNo = `INV-${new Date().toISOString().split('T')[0].replace(/-/g,'')}-${String(o.order_id).replace(/\D/g,'').slice(-6) || '123456'}`;
...
<head>
  <meta charset="UTF-8"/>
  <title>Tax Invoice - ${o.order_id}</title>
  <style>
    body { font-family: 'Inter', system-ui, sans-serif; padding: 40px; color: #000; max-width: 800px; margin: 0 auto; background: #fff; }
    .header { display: flex; justify-content: space-between; border-bottom: 2px solid #000; padding-bottom: 16px; margin-bottom: 24px; }
    .title { font-size: 24px; font-weight: 900; letter-spacing: -0.5px; text-transform: lowercase; }
    .tax-inv { text-align: right; font-weight: 800; font-size: 14px; color: #444; }
    .grid { display: grid; grid-template-columns: 1fr 1fr; gap: 24px; margin-bottom: 24px; border: 1px solid #000; padding: 16px; border-radius: 4px; }
    .box-title { font-size: 11px; font-weight: 800; color: #666; text-transform: uppercase; margin-bottom: 6px; }
    table { width: 100%; border-collapse: collapse; margin-top: 24px; border: 1px solid #000; }
    th, td { padding: 10px; border-bottom: 1px solid #000; border-right: 1px solid #000; text-align: left; font-size: 12px; }
    th:last-child, td:last-child { border-right: none; }
    th { background: #f5f5f5; font-size: 11px; font-weight: 800; text-transform: uppercase; color: #333; }
    .totals { margin-top: 24px; width: 320px; margin-left: auto; border: 1px solid #000; border-collapse: collapse; }
    .tot-row { display: flex; justify-content: space-between; padding: 8px 12px; border-bottom: 1px solid #000; font-size: 12px; }
    .tot-row:last-child { border-bottom: none; }
    .tot-grand { font-weight: 900; font-size: 14px; background: #f9fafb; }
    .prob-box { background: #fff7ed; border: 1px solid #fdba74; color: #c2410c; padding: 6px 10px; border-radius: 6px; font-weight: 700; font-size: 11px; margin-top: 8px; }
  </style>
</head>
<body>
  <div class="header">
    <div>
      <div class="title">triven wellness</div>
      <div style="font-size:11px;color:#444;margin-top:4px;line-height:1.4;">116RPN/ 341(7), Harmilap Society, Rana Pratap Nagar, Keshavpuram, Kalyanpur, Kanpur<br/>Phone: 9598182837 • GSTIN: 09AAZFT9024A1ZS • geotriven@gmail.com</div>
    </div>
    <div class="tax-inv">
      TAX INVOICE<br/>
      <span style="font-size:12px;font-weight:600;color:#111;">${invNo}</span>
    </div>
  </div>
  ...
  <div class="grid">
    <div>
      ...
      <div style="font-weight:800;font-size:14px;">${o.billing_customer_name || 'Customer'}</div>
      <div style="font-size:12px;color:#333;margin-top:2px;">${[o.billing_address, o.billing_city, o.billing_state, o.billing_pincode].filter(Boolean).join(', ')}</div>
      <div style="font-size:12px;color:#333;margin-top:2px;">Phone no: <strong>${o.billing_phone || '—'}</strong></div>
      ${probText ? `<div class="prob-box">Problem / Complaint: ${probText}</div>` : ''}
    </div>
    <div>
      <div class="box-title">Order Information</div>
      <div style="font-size:12px;margin-bottom:4px;">INVOICE NO: <strong>${invNo}</strong></div>
      <div style="font-size:12px;margin-bottom:4px;">INVOICE DATE: <strong>${orderDateStr}</strong></div>
      <div style="font-size:12px;margin-bottom:4px;">ORDER ID: <strong>#${o.order_id}</strong></div>
      <div style="font-size:12px;">AWB NUMBER: <strong>${o.awb_code || '—'}</strong></div>
    </div>
  </div>
    ...
    <tbody>
      <tr>
        <td>
          ...
          ${probText ? `<div style="font-size:11px;color:#c2410c;margin-top:3px;font-weight:700;">Problem / Size: ${probText}</div>` : ''}
        </td>
        <td style="text-align:center;">1</td>
        <td style="text-align:right;">₹${subTotal}</td>
        <td style="text-align:right;">₹${subTotal}</td>
        <td style="text-align:right;font-weight:800;">₹${subTotal}</td>
      </tr>
    </tbody>
  </table>
  ...
  <div style="display:flex;justify-content:flex-end;margin-top:20px;">
    <div class="totals">
      <div class="tot-row"><span>Sub-Total</span><span>₹${subTotal}</span></div>
      <div class="tot-row"><span>Discount</span><span>₹0.00</span></div>
      <div class="tot-row"><span>Tax</span><span>₹0.00</span></div>
      <div class="tot-row tot-grand"><span>Net Amount</span><span>₹${subTotal}</span></div>
    </div>
  </div>

  <div style="margin-top:50px;text-align:center;font-size:11px;color:#888;">POWERED BY ShipMaxx</div>
  <script>window.print();</script>
    ...
    if (setActionId) setActionId(null);
  }
};

// ── Order Detail Modal ────────────────────────────────────────────────────────
function OrderDetailModal({ order, tracking, trackingLoading, fetchLiveTracking, onClose, actionId, setActionId, onPrescription }) {
  const { user } = useAuth();
    ...
    try {
      ...
      if (res.data && res.data.size > 0) {
        ...
        if (!isPdf && !isHtml) {
          try {
            const parsedJson = JSON.parse(fullText);
            throw new Error(parsedJson.message || parsedJson.detail || `${type} not available yet.`);
          } catch (e) {
            if (e.message && !e.message.includes('JSON')) throw e;
      ...
      <div className={`bg-white rounded-3xl shadow-xl w-full ${inlinePdfUrl ? 'max-w-5xl min-h-[75vh]' : 'max-w-3xl'} max-h-[90vh] overflow-hidden flex flex-col transition-all duration-300`}>
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-gray-50 rounded-2xl p-4 space-y-2 text-xs text-gray-600">
              <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Shipment Details</h4>
              <div className="flex items-center gap-2 mb-1">
                <span className="font-bold text-gray-400">Department:</span>
                {(() => {
                  const dept = getShipmaxxDept(order);
                  let pTag = 'Male';
                  let pClass = 'bg-blue-50 text-blue-700 border-blue-200';
                  if (dept === 'skin') { pTag = 'Skin'; pClass = 'bg-pink-50 text-pink-700 border-pink-200'; }
                  else if (dept === 'ortho') { pTag = 'Ortho'; pClass = 'bg-green-50 text-green-700 border-green-200'; }
                  return <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md border ${pClass}`}>{pTag}</span>;
                })()}
              </div>
              ...
              <p><span className="font-bold text-gray-400">Product: </span><span className="font-semibold text-gray-800">{(order.order_items && order.order_items[0]?.name) || (order.products && order.products[0]?.name) || order.medicine || order.title || '—'}</span></p>
              {(order.problem || order.lead_id?.problem) && (
                <p><span className="font-bold text-amber-600">Problem: </span><span className="font-medium text-amber-800">{order.problem || order.lead_id?.problem}</span></p>
              )}
            </div>
          </div>
          {order.awb_code && (
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-2 bg-gray-50/40 shrink-0">
          ...
          {['admin', 'manager', 'doctor'].includes(user?.role) && (
            <button
              onClick={() => onPrescription && onPrescription(order)}
              className="px-4 py-2 rounded-xl text-xs font-semibold bg-sky-600 hover:bg-sky-700 text-white transition flex items-center gap-1"
          ...
          <button onClick={() => openInvoice(order.order_id, setActionId, order)} disabled={actionId === `invoice-${order.order_id}`}
        ...
        <form onSubmit={e => { e.preventDefault(); setPage(1); fetchOrders(); }} className="px-5 py-4 space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <Field label="Department">
              <select className={inp} value={deptFilter} onChange={e => setDeptFilter(e.target.value)}>
                <option value="all">All Depts (Male, Ortho, Skin)</option>
          ...
          <div className="flex gap-2 pt-2 border-t border-gray-50">
            ...
            <button type="button" onClick={() => { setSearch(''); setStatus('all'); setDeptFilter('all'); setFromDate(''); setToDate(''); setPage(1); }} className="px-4 py-2 rounded-xl bg-gray-100 text-gray-600 text-xs font-semibold hover:bg-gray-200 transition">Reset</button>
          </div>
        </form>
      </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  {displayOrders.map(o => {
                    const dept = getShipmaxxDept(o);
                    let pTag = 'Male';
                    let pClass = 'bg-blue-50 text-blue-700 border-blue-200';
                    if (dept === 'skin') { pTag = 'Skin'; pClass = 'bg-pink-50 text-pink-700 border-pink-200'; }
                    else if (dept === 'ortho') { pTag = 'Ortho'; pClass = 'bg-green-50 text-green-700 border-green-200'; }
                    
                    const firstItemName = (o.order_items && o.order_items[0]?.name) || (o.products && o.products[0]?.name) || '';
                    ...
                    <tr key={o.order_id} className="hover:bg-gray-50/40 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-col gap-0.5 items-start max-w-[160px]">
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md border ${pClass}`}>{pTag}</span>
                          {firstItemName && <span className="text-[11px] font-semibold text-gray-700 truncate w-full" title={firstItemName}>{firstItemName}</span>}
                          {(o.problem || o.lead_id?.problem) && (
                            <span className="text-[10px] text-amber-700 font-medium truncate w-full" title={o.problem || o.lead_id?.problem}>
                              Prob: {o.problem || o.lead_id?.problem}
                            </span>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-center gap-1.5">
                          <button onClick={() => handleViewDetails(o)} className="px-2 py-1 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 text-[11px] font-semibold transition">View</button>
                          {['admin', 'manager', 'doctor'].includes(user?.role) && (
                            <button
                              onClick={() => {
                                setSelectedRxOrder({
                                  ...o,
                                  patientName: o.billing_customer_name,
                                  mobile: o.billing_phone,
                                  pid: o.order_id,
                                  amount: o.sub_total,
                                  department: dept,
                                  problem: o.problem || o.lead_id?.problem,
                                });
                                setRxModalOpen(true);
                              }}
                              className="px-2 py-1 rounded-lg bg-sky-600 hover:bg-sky-700 text-white text-[11px] font-semibold transition flex items-center gap-1"
                              title="Generate Department Prescription"
                            >
                              <span>Rx</span>
                            </button>
                          )}
                          <button onClick={() => openInvoice(o.order_id, setActionId, o)} disabled={actionId === `invoice-${o.order_id}`} className="px-2 py-1 rounded-lg bg-orange-500 hover:bg-orange-600 text-white text-[11px] font-semibold transition disabled:opacity-50" title="Generate Invoice">
                            {actionId === `invoice-${o.order_id}` ? 'Invoice…' : 'Invoice'}
                          </button>
                          {o.awb_code && o.status !== 'CANCELLED' && (
                            <button onClick={() => handleCancel(o.awb_code)} className="px-2 py-1 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 text-[11px] font-semibold transition">Cancel</button>
                          )}
                        </div>
                      </td>
                    </tr>
...
export default function Shipmaxx() {
  ...
  const [modalDetailOrder, setModalDetailOrder] = useState(null);
  const [modalTracking, setModalTracking] = useState(null);
  const [modalTrackLoading, setModalTrackLoading] = useState(false);
  const [modalActionId, setModalActionId] = useState(null);

  const handleOpenModalDetails = async (order) => {
    setModalDetailOrder(order);
    if (order.awb_code) {
      setModalTrackLoading(true);
      try { const res = await smxSvc.trackShipment(order.awb_code); setModalTracking(res.data?.data || res.data); } catch { }
      finally { setModalTrackLoading(false); }
    }
  };
  ...
  const [maleOrdersList, setMaleOrdersList] = useState([]);
  const [orthoOrdersList, setOrthoOrdersList] = useState([]);
  const [skinOrdersList, setSkinOrdersList] = useState([]);
  ...
  useEffect(() => {
    ...
    const fetchTodayCount = async () => {
      try {
        ...
        const res = await smxSvc.getOrders({ from: today, to: today, limit: 5000 });
        const orders = res.data?.data?.data || res.data?.data || [];
        
        let male = [];
        let ortho = [];
        let skin = [];
        const validDeptOrders = [];
        orders.forEach(o => {
          const dept = getShipmaxxDept(o);
          if (dept === 'male') { male.push(o); validDeptOrders.push(o); }
          else if (dept === 'ortho') { ortho.push(o); validDeptOrders.push(o); }
          else if (dept === 'skin') { skin.push(o); validDeptOrders.push(o); }
        });
        
        setTodayCount(validDeptOrders.length);
        setTodayOrdersList(validDeptOrders);
        setMaleCount(male.length);
        setMaleOrdersList(male);
        setOrthoCount(ortho.length);
        setOrthoOrdersList(ortho);
        setSkinCount(skin.length);
        setSkinOrdersList(skin);
      } catch (e) {
  ...
  useEffect(() => {
    ...
    let mappedProduct = 'Male Wellness';
    if (dept === 'ortho' || tLow.includes('ortho')) {
      mappedProduct = 'Ortho Wellness';
    } else if (dept === 'skin' || tLow.includes('skin')) {
      mappedProduct = 'Skin Wellness';
    } else if (dept === 'male' || tLow.startsWith('call ') || rawTitle.trim() === fullName) {
      mappedProduct = 'Male Wellness';
    }
    ...
    if (step === 0) return (
      <div className="space-y-5 max-w-xl animate-in fade-in duration-200">
        <div className="bg-slate-50 border-2 border-slate-200 rounded-2xl p-4 flex items-center justify-between">
          <div>
            <h4 className="font-black text-slate-900 text-sm">ShipMaxx Logistics Authentication</h4>
            <p className="text-xs font-bold text-slate-500 mt-0.5">Authenticate with email/password or bearer API key to run order & shipment API tasks</p>
          </div>
          {token ? (
            <span className="text-xs font-black px-3 py-1 rounded-full bg-emerald-100 text-emerald-800 border border-emerald-300 shrink-0 flex items-center gap-1">
              <svg className="w-3.5 h-3.5 text-emerald-700" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path d="M20 6L9 17l-5-5"/></svg>
              Connected
            </span>
          ) : (
            <span className="text-xs font-black px-3 py-1 rounded-full bg-amber-100 text-amber-900 border border-amber-300 shrink-0 flex items-center gap-1">
              <svg className="w-3.5 h-3.5 text-amber-700" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>
              Action Required
            </span>
          )}
        </div>

        <div className="bg-white rounded-2xl shadow-sm border-2 border-slate-200 overflow-hidden space-y-0">
          <div className="h-1.5 bg-blue-600" />
          
          {/* Option 1: Email & Password */}
          <div className="px-5 py-4 space-y-3.5">
            <div className="flex items-center justify-between">
              <h3 className="font-black text-slate-900 text-sm tracking-tight">Option 1: Email & Password Credentials</h3>
              <span className="text-[10px] font-black uppercase text-blue-700 bg-blue-50 px-2 py-0.5 rounded border border-blue-200">Recommended</span>
            </div>
            <Field label="Base API URL">
              <input className={inp} value={authBaseUrl} onChange={e => setAuthBaseUrl(e.target.value)} />
            </Field>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <Field label="Email Address">
                <input className={inp} placeholder="admin@example.com" value={authEmail} onChange={e => { setAuthEmail(e.target.value); localStorage.setItem('smx_auth_email', e.target.value); }} />
              </Field>
              <Field label="Password">
                <input className={inp} type="password" placeholder="••••••••" value={authPassword} onChange={e => { setAuthPassword(e.target.value); localStorage.setItem('smx_auth_pass', e.target.value); }} />
              </Field>
            </div>
            <button onClick={() => call(async () => {
              if (!authEmail || !authPassword) throw new Error('Please enter Email & Password');
              const res = await smxSvc.login(authEmail, authPassword, '', authBaseUrl);
              const t = res.data?.data?.token || res.data?.token || 'active';
              if (t) { saveToken(t); setToken(t); }
              return res.data;
            })} className="w-full py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-black shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-2 active:scale-[0.99]">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z"/></svg>
              Login with Email & Password
            </button>
          </div>

          {/* Option 2: Bearer Token */}
          <div className="border-t-2 border-slate-100 bg-slate-50/80 px-5 py-4 space-y-3.5">
            <div className="flex items-center justify-between">
              <h3 className="font-black text-slate-900 text-sm tracking-tight">Option 2: Direct Bearer Token</h3>
              <span className="text-[10px] font-black uppercase text-slate-600 bg-slate-200 px-2 py-0.5 rounded">Alternative</span>
            </div>
            <Field label="API Key (Bearer Token)">
              <input className={inp} type="password" placeholder="Paste Bearer Token starting with eyJ..." value={authApiKey} onChange={e => setAuthApiKey(e.target.value)} />
            </Field>
            <button onClick={() => call(async () => {
              if (!authApiKey) throw new Error('Please paste a Bearer Token');
              saveToken(authApiKey);
              setToken(authApiKey);
              return { message: "Token set successfully!" };
            })} className="w-full py-2.5 rounded-xl bg-slate-800 hover:bg-slate-900 text-white text-xs font-black shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-2 active:scale-[0.99]">
              <svg className="w-4 h-4 text-amber-400" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>
        ...
        {token && (
          <div className="flex items-center justify-between bg-emerald-50 border-2 border-emerald-200 rounded-xl p-4">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full bg-emerald-600 text-white flex items-center justify-center font-bold text-sm shrink-0">✓</div>
              <div>
                <p className="text-emerald-950 text-sm font-black">ShipMaxx API Token Active</p>
                <p className="text-emerald-700 text-xs font-bold">Valid for 24 hours · Ready to create orders & generate labels</p>
              </div>
            </div>
            <button onClick={() => goStep(1)} className="px-4 py-2 rounded-xl bg-emerald-700 hover:bg-emerald-800 text-white text-xs font-black shadow transition-all">
          ...
          <div className="px-5 py-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
            <Field label="SKU *">
              <select className={inp} value={order.products[0].sku} onChange={e => setProduct('sku', e.target.value)}>
                ...
                <option value="Male Wellness">Male Wellness</option>
                <option value="Ortho Wellness">Ortho Wellness</option>
    ...
    <div className="space-y-4 p-1 sm:p-2">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 overflow-x-auto pb-1 scrollbar-hide shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div onClick={() => setClickedList({ title: 'Male Wellness', orders: maleOrdersList })} className="font-bold text-gray-700 bg-white px-3 py-1.5 rounded-xl border border-gray-200 shadow-sm text-sm flex items-center gap-2 w-fit cursor-pointer hover:bg-gray-50 hover:border-blue-300 transition select-none">
            <span className="uppercase tracking-wide text-xs text-gray-500">Male</span>
            <span className="text-blue-700 bg-blue-50 px-2.5 py-0.5 rounded-lg text-lg border border-blue-100">{maleCount}</span>
          </div>
          <div onClick={() => setClickedList({ title: 'Ortho Wellness', orders: orthoOrdersList })} className="font-bold text-gray-700 bg-white px-3 py-1.5 rounded-xl border border-gray-200 shadow-sm text-sm flex items-center gap-2 w-fit cursor-pointer hover:bg-gray-50 hover:border-green-300 transition select-none">
            <span className="uppercase tracking-wide text-xs text-gray-500">Ortho</span>
            <span className="text-green-700 bg-green-50 px-2.5 py-0.5 rounded-lg text-lg border border-green-100">{orthoCount}</span>
          </div>
          <div onClick={() => setClickedList({ title: 'Skin Wellness', orders: skinOrdersList })} className="font-bold text-gray-700 bg-white px-3 py-1.5 rounded-xl border border-gray-200 shadow-sm text-sm flex items-center gap-2 w-fit cursor-pointer hover:bg-gray-50 hover:border-pink-300 transition select-none">
            <span className="uppercase tracking-wide text-xs text-gray-500">Skin</span>
            <span className="text-pink-700 bg-pink-50 px-2.5 py-0.5 rounded-lg text-lg border border-pink-100">{skinCount}</span>
          </div>
        </div>
      ...
      {section === 'actions' && (
        <>
          ...
          {/* Step Content */}
          <div className="bg-white rounded-2xl border-2 border-slate-200/80 p-6 shadow-sm space-y-5 mt-2">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <div className="flex items-center gap-3">
                <span className="w-8 h-8 rounded-xl bg-blue-600 text-white text-sm font-black flex items-center justify-center shadow-sm">
                  {step + 1}
                </span>
                <div>
                  <h3 className="font-black text-slate-900 text-base tracking-tight">{STEPS[step]}</h3>
                  <p className="text-xs font-bold text-slate-500">Step {step + 1} of 6 · ShipMaxx Logistics Workflow</p>
                </div>
              </div>
              {token ? (
                <span className="text-[11px] font-extrabold px-3 py-1 rounded-full bg-blue-50 text-blue-700 border border-blue-200 flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-blue-500 animate-pulse" />
                  API Active
                </span>
              ) : (
                <button onClick={() => goStep(0)} className="text-[11px] font-extrabold px-3 py-1 rounded-full bg-amber-50 text-amber-800 border border-amber-300 hover:bg-amber-100 transition-all inline-flex items-center gap-1">
                  <svg className="w-3.5 h-3.5 text-amber-700" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z"/></svg>
                  Authenticate API
                </button>
              )}
            </div>
            ...
            {result && !error && !(result.data?.label_url === null) && (
              <div className="flex items-center gap-2 bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3">
                <svg className="w-4 h-4 text-emerald-600 shrink-0" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path d="M9 12l2 2 4-4"/><circle cx="12" cy="12" r="10"/></svg>
                <span className="text-emerald-800 text-sm font-extrabold">Operation Completed Successfully</span>
              </div>
            )}
          </div>
        </>
      ...
      {clickedList && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-xl w-full max-w-3xl max-h-[85vh] flex flex-col">
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between shrink-0">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">Click Generate Invoice or View Details for any order</p>
              </div>
              ...
              {clickedList.orders.length === 0 ? (
                ...
                <div className="space-y-3">
                  {clickedList.orders.map(o => {
                    ...
                    const itemName = (o.order_items && o.order_items[0]?.name) || (o.products && o.products[0]?.name) || o.medicine || o.title || '';
                    const prob = o.problem || o.lead_id?.problem || '';

                    return (
                      <div key={o.order_id} className="p-4 border border-gray-200/80 rounded-2xl bg-white shadow-sm flex flex-col gap-3 transition-all hover:border-gray-300">
                          ...
                          <div className="flex flex-wrap sm:flex-col items-start sm:items-end gap-1.5 shrink-0">
                            ...
                            {o.verification_staff_id && (
                              <div className="flex flex-col items-start sm:items-end text-right">
                                <span className="text-[9px] font-bold text-gray-400 uppercase tracking-wider">Verified By</span>
                        ...
                        {/* Order Details: ID, AWB, Product & Problem */}
                        <div className="bg-gray-50/70 border border-gray-100 rounded-xl p-3 space-y-1.5 text-xs">
                          <div className="flex flex-wrap items-center gap-2">
                            <div className="bg-white border border-gray-200 text-blue-700 px-2 py-0.5 rounded flex items-center gap-1.5 text-[10px] font-mono font-bold uppercase">
                              <span className="text-gray-400">Order ID:</span> {o.order_id}
                            </div>
                            {o.awb_code && (
                              <div className="bg-white border border-gray-200 text-purple-700 px-2 py-0.5 rounded flex items-center gap-1.5 text-[10px] font-mono font-bold uppercase">
                                <span className="text-gray-400">AWB:</span> {o.awb_code}
                              </div>
                            )}
                            {o.sub_total > 0 && (
                              <div className="bg-emerald-50 border border-emerald-200 text-emerald-800 px-2 py-0.5 rounded font-black text-[10px]">
                                ₹{o.sub_total}
                              </div>
                            )}
                          </div>
                          ...
                          {itemName && (
                            <div className="flex items-center gap-1.5">
                              <span className="font-bold text-gray-400 text-[11px]">Product / Items:</span>
                              <span className="font-semibold text-gray-800">{itemName}</span>
                            </div>
                          )}

                          {prob && (
                            <div className="flex items-start gap-1.5 text-amber-800 bg-amber-50/80 border border-amber-200/60 p-2 rounded-lg">
                              <span className="font-bold text-[10px] uppercase text-amber-600 shrink-0 mt-0.5">Problem:</span>
                              <span className="font-medium text-xs">{prob}</span>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="flex items-center justify-end gap-2 pt-1 border-t border-gray-100">
                          <button
                            onClick={() => openInvoice(o.order_id, setModalActionId, o)}
                            disabled={modalActionId === `invoice-${o.order_id}`}
                            className="px-3 py-1.5 rounded-xl bg-orange-500 hover:bg-orange-600 text-white text-xs font-bold transition disabled:opacity-50 flex items-center gap-1.5 shadow-sm active:scale-95">
                            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                            {modalActionId === `invoice-${o.order_id}` ? 'Generating...' : 'Generate Invoice'}
                          </button>
                          <button
                            onClick={() => handleOpenModalDetails(o)}
                            className="px-3 py-1.5 rounded-xl bg-blue-50 text-blue-600 hover:bg-blue-100 text-xs font-bold transition active:scale-95">
                            View Details
                          </button>
                        </div>
                      </div>
      ...
      <OrderDetailModal
        order={modalDetailOrder} tracking={modalTracking} trackingLoading={modalTrackLoading}
        fetchLiveTracking={async () => { if (!modalDetailOrder?.awb_code) return; setModalTrackLoading(true); try { const r = await smxSvc.trackShipment(modalDetailOrder.awb_code); setModalTracking(r.data?.data || r.data); } catch { } finally { setModalTrackLoading(false); } }}
        onClose={() => { setModalDetailOrder(null); setModalTracking(null); }}
        actionId={modalActionId} setActionId={setModalActionId}
      />
    </div>
  );
}

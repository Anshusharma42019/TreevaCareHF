...
export default function AppointmentBook() {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const payload = { ...form };
      if (!payload.department) delete payload.department;
      await updateAppointment(rightPanel.appt._id, payload);
      setRightPanel(null);
      load();
    } catch (err) {
          ...
          <div className="flex items-center gap-3 flex-wrap">
            {canManage && (
              <select value={filters.department} onChange={e => setFilters(f => ({ ...f, department: e.target.value, page: 1 }))}
                ...
                {(user?.role === 'admin' || user?.role === 'manager' || !user?.departments?.length ? ['male', 'ortho', 'skin'] : user.departments).map(d => <option key={d} value={d}>{d.toUpperCase()}</option>)}
              </select>
            )}

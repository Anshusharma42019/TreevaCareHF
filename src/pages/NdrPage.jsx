...
function NdrList({ department: externalDept, setDepartment: externalSetDept, onSelectNdr, onUseAwb }) {
  ...
  const maleCount = ndrs.filter(n => getDept(n) === 'male').length;
  const orthoCount = ndrs.filter(n => getDept(n) === 'ortho').length;
  const skinCount = ndrs.filter(n => getDept(n) === 'skin').length;
    ...
    <div className="bg-white rounded-2xl shadow-sm overflow-hidden" style={{ border: '1px solid rgba(0,0,0,0.06)' }}>
      ...
      <div className="px-5 py-3 border-b border-gray-100 space-y-3 bg-gray-50/30">
        ...
        <div className="flex items-center gap-2 flex-wrap py-1">
          ...
          >
            <span>All</span>
            <span className={`px-1.5 py-0.2 rounded-md text-[10px] ${department === 'all' ? 'bg-red-700 text-white' : 'bg-gray-100 text-gray-600'}`}>{ndrs.length}</span>
          </button>
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between sm:justify-end gap-3 pb-1 scrollbar-hide">
        <select
          ...
          className="h-11 px-3.5 rounded-xl border border-gray-200 bg-white text-xs font-bold text-gray-700 shadow-sm focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 cursor-pointer uppercase tracking-wider transition hover:border-gray-300 shrink-0"
        >
      ...
      {tab === 'board'  && <OrderStatusBoard title="Undelivered Orders" department={department} defaultStatus="UNDELIVERED_1ST_ATTEMPT" allowedStatuses={['NEW', 'PICKUP_SCHEDULED', 'SHIPPED', 'IN_TRANSIT', 'OUT_FOR_DELIVERY', 'DELIVERED', 'UNDELIVERED_1ST_ATTEMPT', 'UNDELIVERED_2ND_ATTEMPT', 'UNDELIVERED_3RD_ATTEMPT', 'UNDELIVERED']} />}

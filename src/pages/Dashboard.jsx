  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: `0 4px 14px -2px rgba(15, 23, 42, 0.08), 0 2px 8px -1px ${color}30`,
        minHeight: 125,
      }}
      onMouseEnter={e => {
        e.currentTarget.style.boxShadow = `0 12px 30px -4px ${color}45`;
        e.currentTarget.style.transform = 'translateY(-4px)';
        e.currentTarget.style.borderColor = `${color}80`;
      }}
      onMouseLeave={e => {
        e.currentTarget.style.boxShadow = `0 4px 14px -2px rgba(15, 23, 42, 0.08), 0 2px 8px -1px ${color}30`;
        e.currentTarget.style.transform = 'none';
        e.currentTarget.style.borderColor = `${color}50`;
      }}
    >
      {/* Ambient glow in corner */}
      <div className="absolute -right-6 -bottom-6 w-32 h-32 rounded-full blur-2xl group-hover:scale-150 transition-transform duration-700 pointer-events-none" style={{ background: color, opacity: 0.14 }}></div>
      {/* Glass reflection */}
      <div className="absolute inset-0 bg-gradient-to-b from-white/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"></div>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', position: 'relative', zIndex: 10, width: '100%', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <div style={{ fontSize: 13, fontWeight: 900, color: '#0f172a', textTransform: 'uppercase', letterSpacing: 0.6 }}>{label}</div>
          {subtext && (
            <div title={subtext} style={{ background: '#cbd5e1', color: '#0f172a', padding: '2px', borderRadius: '50%', cursor: 'help' }}>
              {icons.info}
            </div>
          )}
        </div>
        <div style={{ width: 34, height: 34, borderRadius: '50%', background: `${color}20`, border: `1.5px solid ${color}50`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }} className="group-hover:scale-110 transition-transform">
          {icon ? <span style={{ color }}>{icon}</span> : <div style={{ width: 7, height: 7, borderRadius: '50%', background: color, boxShadow: `0 0 10px ${color}` }}></div>}
        </div>
      </div>

      <div style={{ position: 'relative', zIndex: 10, marginTop: 14 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
          <div style={{ fontSize: 34, fontWeight: 900, color: color, lineHeight: 1, letterSpacing: '-0.025em' }}>
            {value !== undefined && value !== null ? value : <span style={{ opacity: 0.4, fontSize: 20 }}>—</span>}
          </div>
          {unit && <span style={{ fontSize: 11, fontWeight: 900, opacity: 0.95, textTransform: 'uppercase', letterSpacing: 1, color }}>{unit}</span>}
        </div>

        {(progress !== undefined || progressLabel) && (
          <div style={{ marginTop: 10 }}>
            {progress !== undefined && (
              <div style={{ width: '100%', background: 'rgba(0,0,0,0.1)', borderRadius: 4, height: 6, overflow: 'hidden', marginBottom: 6 }}>
            ...
            {progressLabel && <div style={{ fontSize: 12, fontWeight: 800, color: '#1e293b', textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: '4px' }}>{progressLabel}</div>}
          </div>
        )}
      </div>

      {/* Accent line at bottom */}
      <div className="absolute bottom-0 left-0 right-0 h-1.5 transition-opacity pointer-events-none group-hover:opacity-70" style={{ background: color, opacity: 0.4 }}></div>
    </div>
  );
}


/* ─── Section Card Wrapper with Optional SVG Icon & Section Tag ──────────────── */
function SectionCard({ title, subtitle, icon, tag, accentColor = '#16a34a', children, action }) {
  return (
    <div style={{
      background: '#ffffff',
      borderRadius: 16,
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 10, paddingBottom: 12, borderBottom: '1.5px solid #f1f5f9' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            {tag && (
              <span style={{ fontSize: 10, fontWeight: 900, color: accentColor, textTransform: 'uppercase', letterSpacing: 0.9, display: 'block', marginBottom: 2 }}>
                {tag}
              </span>
            )}
            <div style={{ fontSize: 18, fontWeight: 900, color: '#0f172a', fontFamily: "'Outfit', sans-serif", letterSpacing: '-0.02em', lineHeight: 1.2 }}>
              {title}
            </div>
            {subtitle && <div style={{ fontSize: 12.5, color: '#475569', marginTop: 3, fontWeight: 600 }}>{subtitle}</div>}
          </div>
        </div>
        {action}
      </div>
...
function DashboardFilterBar({ datePreset, onSelectPreset, filterFrom, setFilterFrom, filterTo, setFilterTo, department, setDepartment, canManage, lastUpdated, onRefresh, autoRefresh, onToggleAutoRefresh, onDownloadCSV, csvLoading, t }) {
  return (
    <div className="no-print" style={{
      ...
      boxShadow: '0 2px 10px rgba(0,0,0,0.04)', marginBottom: 22,
    }}>
      {/* Date preset pills */}
      <div style={{ display: 'flex', gap: 4, background: '#f1f5f9', borderRadius: 10, padding: 4, border: '1px solid #cbd5e1', flexWrap: 'wrap' }}>
        {DATE_FILTERS.map(f => (
          <button key={f.id} onClick={() => onSelectPreset(f.id)}
            style={{
              padding: '6px 14px', borderRadius: 8, border: 'none', fontSize: 12, fontWeight: 800, cursor: 'pointer',
              background: datePreset === f.id ? '#16a34a' : 'transparent',
              color: datePreset === f.id ? '#ffffff' : '#334155',
              transition: 'all .15s',
              boxShadow: datePreset === f.id ? '0 1px 4px rgba(22,163,74,0.3)' : 'none',
            }}
            className={datePreset === f.id ? '' : 'hover:bg-slate-200/80 hover:text-slate-900'}
          >
            {t(f.label)}
          </button>
        ))}
      ...
      {datePreset === 'custom' && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <input type="date" value={filterFrom} onChange={e => setFilterFrom(e.target.value)}
            style={{ padding: '6px 10px', borderRadius: 8, border: '1px solid #cbd5e1', fontSize: 12, background: '#fff', fontWeight: 700, color: '#0f172a' }} />
          <span style={{ color: '#475569', fontSize: 12, fontWeight: 700 }}>to</span>
      ...
      {canManage && (
        <div style={{ display: 'flex', alignItems: 'center', background: '#f1f5f9', border: '1px solid #cbd5e1', borderRadius: 10, padding: '0 10px' }}>
          <span style={{ color: '#475569' }}>{icons.building}</span>
          <select
            value={department}
            onChange={e => setDepartment(e.target.value)}
            style={{ padding: '7px 8px', border: 'none', fontSize: 12, background: 'transparent', fontWeight: 800, color: '#0f172a', outline: 'none', cursor: 'pointer' }}
          >
      ...
      <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
        ...
        <button onClick={onRefresh} title="Refresh Dashboard" style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', borderRadius: 8, padding: '7px 12px', cursor: 'pointer', color: '#1e293b', display: 'flex', alignItems: 'center', transition: 'background 0.15s' }} className="hover:bg-slate-200">
        ...
        <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 800, color: '#1e293b', cursor: 'pointer', userSelect: 'none' }}>
...
export default function Dashboard() {
  ...
  const maleLeadCount = stats?.departmentLeads?.male || 0;
  const orthoLeadCount = stats?.departmentLeads?.ortho || 0;
  const skinLeadCount = stats?.departmentLeads?.skin || 0;
  const newLeadsTotal = stats?.newLeadsToday || 0;
  const maleLeadPercent = newLeadsTotal > 0 ? Math.round((maleLeadCount / newLeadsTotal) * 100) : 0;
  const orthoLeadPercent = newLeadsTotal > 0 ? Math.round((orthoLeadCount / newLeadsTotal) * 100) : 0;
  const skinLeadPercent = newLeadsTotal > 0 ? Math.round((skinLeadCount / newLeadsTotal) * 100) : 0;
  const overallConversionRate = stats?.conversionRate || 0;
  const maleConversionRate = stats?.maleConversionRate || 0;
  const orthoConversionRate = stats?.orthoConversionRate || 0;
  const skinConversionRate = stats?.skinConversionRate || 0;
  const maleConverted = stats?.maleConverted || 0;
  const orthoConverted = stats?.orthoConverted || 0;
  const skinConverted = stats?.skinConverted || 0;

  // Funnel calculation
  const maxFunnelCount = Math.max(...(stats?.salesFunnel || []).map(f => Number(f?.count) || 0), 1);
  const maxSourceCount = Math.max(...(stats?.sourcePerformance || []).map(s => Number(s?.count) || 0), 1);
  const hasFunnelData = (stats?.salesFunnel || []).some(f => (Number(f?.count) || 0) > 0);
  const hasSourceData = (stats?.sourcePerformance || []).some(s => (Number(s?.count) || 0) > 0);
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '16px 16px' }}>
        ...
        <div className="no-print" style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14, flexWrap: 'wrap', gap: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ width: 44, height: 44, borderRadius: 12, background: 'linear-gradient(135deg, #16a34a, #15803d)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', boxShadow: '0 4px 12px rgba(22,163,74,0.25)', flexShrink: 0 }}>
                {icons.dashboard}
              </div>
              <div>
                <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight" style={{ margin: 0, fontFamily: "'Outfit', sans-serif" }}>
                  Sales & Leads Dashboard
                </h1>
                <p className="text-xs md:text-sm font-semibold text-slate-500 mt-0.5" style={{ margin: 0 }}>
                  Company-wide dynamic analytics of lead intake, conversions, and operational volume ({getPeriodLabel()})
                </p>
              </div>
            </div>
            ...
            <div className="text-right hidden sm:block">
              ...
              <div className="text-xs font-bold text-emerald-700 mt-0.5 flex items-center justify-end gap-1">
          ...
          {/* Top Row KPI Cards - Spans cleanly across the entire row for all users */}
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 pt-2 border-t border-slate-100">
            {/* 1. Overall VR Rate */}
            <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', padding: '10px 14px', borderRadius: 14, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
            ...
            {/* 2. New Leads */}
            <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', padding: '10px 14px', borderRadius: 14, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
            ...
            <div style={{ background: '#fffbe6', border: '1px solid #fef08a', padding: '10px 14px', borderRadius: 14, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
              <span style={{ fontSize: 10, fontWeight: 800, color: '#d97706', textTransform: 'uppercase', letterSpacing: 0.5 }}>Pending QA</span>
              <span style={{ fontSize: 22, fontWeight: 900, color: '#b45309', lineHeight: 1.1, marginTop: 2 }}>
                {stats?.pendingVerificationsCount ?? 0}
              </span>
            </div>

            {/* 4. Ready to Ship */}
            <div style={{ background: '#f3e8ff', border: '1px solid #e9d5ff', padding: '10px 14px', borderRadius: 14, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
              <span style={{ fontSize: 10, fontWeight: 800, color: '#7c3aed', textTransform: 'uppercase', letterSpacing: 0.5 }}>Ready to Ship</span>
              <span style={{ fontSize: 22, fontWeight: 900, color: '#6b21a8', lineHeight: 1.1, marginTop: 2 }}>
                {stats?.readyToShipmentCount ?? 0}
              </span>
            </div>

            {/* 5. Verified Count */}
            <div style={{ background: '#ecfdf5', border: '1px solid #a7f3d0', padding: '10px 14px', borderRadius: 14, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
              <span style={{ fontSize: 10, fontWeight: 800, color: '#059669', textTransform: 'uppercase', letterSpacing: 0.5 }}>Verified ({getPeriodLabel()})</span>
              <span style={{ fontSize: 22, fontWeight: 900, color: '#047857', lineHeight: 1.1, marginTop: 2 }}>
                {stats?.readyToShipCreatedCount ?? 0}
              </span>
            </div>

            {/* 6. Delivered Revenue */}
            <div style={{ background: '#fdf4ff', border: '1px solid #f5d0fe', padding: '10px 14px', borderRadius: 14, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
              <span style={{ fontSize: 10, fontWeight: 800, color: '#c026d3', textTransform: 'uppercase', letterSpacing: 0.5 }}>Delivered Revenue</span>
        ...
        <div className="no-print" style={{
          ...
          background: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)',
          borderRadius: 16, padding: '6px 8px', border: '1px solid rgba(0,0,0,0.08)',
          flexWrap: 'wrap', alignItems: 'center', boxShadow: '0 2px 10px rgba(0,0,0,0.04)',
        }}>
          {tabsList.map(tab => {
            ...
            return (
              <button
                ...
                style={{
                  padding: '10px 22px', borderRadius: 12, border: 'none', fontSize: 13, fontWeight: 800, cursor: 'pointer',
                ...
                className={isSelected ? '' : 'hover:bg-slate-100 hover:text-slate-900'}
              >
                <span style={{ color: isSelected ? '#ffffff' : '#16a34a' }}>{tab.icon}</span>
                {tab.label}
              </button>
            );
          })}
        ...
        {activeTab === 'overview' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <SectionCard tag="SECTION 01 · FAST SHORTCUTS" accentColor="#2563eb" title="Quick Action Shortcuts" subtitle="Fast navigation to primary team management modules" icon={icons.lightning}>
            ...
            {user?.role === 'sales' && (
              <SectionCard tag="SECTION 02 · STAFF PERFORMANCE" accentColor="#10b981" title="My Personal Activity Metrics" subtitle={`Your real-time dynamic performance tallies for ${getPeriodLabel()}`} icon={icons.activity}>
            ...
            {/* Lead Pipeline KPIs */}
            <SectionCard tag="SECTION 02 · LEAD GENERATION & INTAKE" accentColor="#16a34a" title="Dynamic Lead Pipeline & Generation" subtitle={`Real-time distribution of leads across departmental categories (${getPeriodLabel()})`} icon={icons.target}>
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3.5">
                ...
                />
                <OpsKpiCard
                  label={`${t('Male')} ${t('Leads')}`}
                  value={maleLeadCount}
                  color="#3b82f6"
                  unit="MALE"
                  icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="8.5" cy="7" r="4" /></svg>}
                  progress={maleLeadPercent}
                  progressLabel={`${maleLeadPercent}% of new leads · VR: ${maleConversionRate}%`}
                />
                <OpsKpiCard
                  label={`${t('Ortho')} ${t('Leads')}`}
                  value={orthoLeadCount}
                  color="#10b981"
                  unit="ORTHO"
                  ...
                  progress={orthoLeadPercent}
                  progressLabel={`${orthoLeadPercent}% of new leads · VR: ${orthoConversionRate}%`}
                />
                <OpsKpiCard
                  label={`${t('Skin')} ${t('Leads')}`}
                  value={skinLeadCount}
                  color="#ec4899"
                  unit="SKIN"
                  icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" /></svg>}
                  progress={skinLeadPercent}
                  progressLabel={`${skinLeadPercent}% of new leads · VR: ${skinConversionRate}%`}
                />
            ...
            <SectionCard tag="SECTION 03 · VERIFICATION & FULFILLMENT WORKFLOW" accentColor="#8b5cf6" title="Live Verification & Shipment Pipeline" subtitle={`Real-time conversion tracking from sales tasks to QA verifications and logistics dispatch (${getPeriodLabel()})`} icon={icons.activity}>
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3.5">
                <OpsKpiCard
                ...
                />
                <OpsKpiCard
                  label="Male Verifications"
                  value={stats?.maleConverted ?? 0}
                  color="#3b82f6"
                  unit="MALE"
                  icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="8.5" cy="7" r="4" /></svg>}
                  progressLabel={`${stats?.malePendingVerifications ?? 0} pending in QA · ${stats?.maleTaskToVerification ?? 0} added (${getPeriodLabel()})`}
                  onClick={canManage ? () => navigate('/verification') : undefined}
                />
                <OpsKpiCard
                  label="Ortho Verifications"
                  value={stats?.orthoConverted ?? 0}
                  color="#10b981"
                  unit="ORTHO"
                  icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path d="M12 21s6-5.3 6-11a6 6 0 1 0-12 0c0 5.7 6 11 6 11z" /><circle cx="12" cy="10" r="2.5" /></svg>}
                  progressLabel={`${stats?.orthoPendingVerifications ?? 0} pending in QA · ${stats?.orthoTaskToVerification ?? 0} added (${getPeriodLabel()})`}
                  onClick={canManage ? () => navigate('/verification') : undefined}
                />
                <OpsKpiCard
                  label="Skin Verifications"
                  value={stats?.skinConverted ?? 0}
                  color="#ec4899"
                  unit="SKIN"
                  icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" /></svg>}
                  progressLabel={`${stats?.skinPendingVerifications ?? 0} pending in QA · ${stats?.skinTaskToVerification ?? 0} added (${getPeriodLabel()})`}
                  onClick={canManage ? () => navigate('/verification') : undefined}
                />
                <OpsKpiCard
            ...
            {(hasFunnelData || hasSourceData) && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <SectionCard tag="SECTION 04 · PIPELINE STAGES" accentColor="#3b82f6" title="Dynamic Sales Stage Funnel" subtitle={`Conversion status breakdown across all sales pipeline phases (${getPeriodLabel()})`} icon={icons.barChart}>
                  <div className="space-y-3 mt-1">
                    {(stats?.salesFunnel && stats.salesFunnel.length > 0) ? (
                      stats.salesFunnel.map((item, idx) => {
                        const stageKey = item?.stage || '';
                        const info = stageNames[stageKey] || { label: stageKey || 'Other', color: '#64748b', icon: icons.tag };
                        const countVal = Number(item?.count) || 0;
                        const percentOfMax = maxFunnelCount > 0 ? Math.round((countVal / maxFunnelCount) * 100) : 0;
                        const percentOfTotal = newLeadsTotal > 0 ? Math.round((countVal / newLeadsTotal) * 100) : 0;
                        return (
                          <div key={stageKey || idx} className="flex items-center gap-3 py-1.5 px-2 rounded-xl hover:bg-slate-50 transition-colors">
                            ...
                            <div className="flex-1 bg-slate-100 rounded-full h-3 overflow-hidden border border-slate-200/60 shadow-inner">
                              <div
                                style={{ width: `${countVal > 0 ? Math.max(percentOfMax, 6) : 0}%`, background: info.color }}
                                className="h-full rounded-full transition-all duration-700 ease-out shadow-sm"
                              />
                            </div>
                            <div style={{ width: 85 }} className="text-right shrink-0">
                              <span className="font-black text-slate-900 text-sm">{countVal.toLocaleString('en-IN')}</span>
                      ...
                      <div className="py-5 text-center text-slate-400 text-xs font-semibold bg-slate-50/50 rounded-xl border border-dashed border-slate-200">No pipeline stage records in selected timeframe</div>
                    )}
                  </div>
                </SectionCard>

                <SectionCard tag="SECTION 04 · MARKETING SOURCES" accentColor="#0ea5e9" title="Live Acquisition Channel Sources" subtitle={`Dynamic volume attribution by marketing channel (${getPeriodLabel()})`} icon={icons.globe}>
                  <div className="space-y-3 mt-1">
                    {(stats?.sourcePerformance && stats.sourcePerformance.length > 0) ? (
                      stats.sourcePerformance.map((item, idx) => {
                        const style = getSourceStyle(item?.source);
                        const countVal = Number(item?.count) || 0;
                        const barW = maxSourceCount > 0 ? Math.round((countVal / maxSourceCount) * 100) : 0;
                        return (
                          <div key={item?.source || idx} className="flex items-center gap-3 py-1.5 px-2 rounded-xl hover:bg-slate-50 transition-colors">
                            ...
                            <div className="flex-1 bg-slate-100 rounded-full h-3 overflow-hidden border border-slate-200/60 shadow-inner">
                              <div
                                style={{ width: `${countVal > 0 ? Math.max(barW, 6) : 0}%`, background: style.color }}
                                className="h-full rounded-full transition-all duration-700 ease-out shadow-sm"
                              />
                            </div>
                            <div style={{ width: 85 }} className="text-right shrink-0">
                      ...
                      <div className="py-5 text-center text-slate-400 text-xs font-semibold bg-slate-50/50 rounded-xl border border-dashed border-slate-200">No attribution source data recorded in selected timeframe</div>
                    )}
                  </div>
                </SectionCard>
              </div>
            ...
            {/* ═══ Company Call & Activity Pulse ═══ */}
            <SectionCard tag="SECTION 04 · CALL CENTER & DISPOSITION PULSE" accentColor="#ef4444" title="Company-Wide Operational & Call Pulse" subtitle={`Live aggregated staff call and lead engagement outcomes (${getPeriodLabel()})`} icon={icons.megaphone}>
              <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 gap-4">
                ...
                <OpsKpiCard
                  ...
                  color="#dc2626"
                  unit="CLOSED"
                  icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" /><line x1="15" y1="9" x2="9" /><line x1="9" y1="9" x2="15" y2="15" /></svg>}
                  progressLabel={`Discontinued or cancelled leads`}
                />
              </div>
            </SectionCard>

            {/* ═══ Team Workforce & Task Fulfillment ═══ */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <SectionCard tag="SECTION 05 · WORKFORCE ATTENDANCE" accentColor="#10b981" title="Live Workforce & Staff Attendance" subtitle="Real-time check-in status of active company sales & support agents" icon={icons.users}>
                ...
                <SectionCard tag="SECTION 05 · TASK QUEUE & REMINDERS" accentColor="#6366f1" title="Active Staff Task Queue" subtitle={`Pending and overdue follow-up task volume requiring staff attention (${getPeriodLabel()})`} icon={icons.tasks}>
            ...
            <SectionCard tag="SECTION 06 · LOGISTICS & RTO PERFORMANCE" accentColor="#d97706" title="Monthly Shipment Performance & RTO Analytics" subtitle="Real-time delivery rates, RTO tracking, and active in-transit fulfillment pulse across all logistics partners (Current Month)" icon={icons.packageCheck}>
        ...
        {activeTab === 'fulfillment' && canManage && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            ...
            {/* ═══ Live Verification & Shipment Workflow Pipeline ═══ */}
            <SectionCard tag="SECTION 01 · VERIFICATION & SHIPMENT WORKFLOW" accentColor="#8b5cf6" title="Live Verification & Shipment Pipeline" subtitle={`Real-time conversion tracking from sales tasks to QA verifications and logistics dispatch (${getPeriodLabel()})`} icon={icons.activity}>
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3.5">
                <OpsKpiCard
                  label="Task to Verification"
                  value={stats?.taskToVerificationCount ?? 0}
            ...
            <SectionCard tag="SECTION 02 · DELIVERIES OVERVIEW" accentColor="#10b981" title="Delivered Orders Overview" subtitle={`Delivered shipments categorization between new acquisition and repeat reorders (${getPeriodLabel()})`} icon={icons.packageCheck}>
            ...
            <SectionCard tag="SECTION 03 · CONVERSION RATIOS" accentColor="#16a34a" title="Lead Verification & Conversion Rate" subtitle={`Conversion efficiency ratios from initial lead acquisition to verified order (${getPeriodLabel()})`} icon={icons.lightning}>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                ...
                />
                <OpsKpiCard
                  label={`${t('Male Conversion')}`}
                  value={`${maleConversionRate}%`}
                  color="#3b82f6"
                  unit="VR RATE"
                  icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="8.5" cy="7" r="4" /></svg>}
                  progress={maleConversionRate}
                  progressLabel={`${maleConverted} verified / ${maleLeadCount} male leads`}
                />
                <OpsKpiCard
                  label={`${t('Ortho Conversion')}`}
                  value={`${orthoConversionRate}%`}
                  color="#10b981"
                  unit="VR RATE"
                  icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path d="M12 21s6-5.3 6-11a6 6 0 1 0-12 0c0 5.7 6 11 6 11z" /><circle cx="12" cy="10" r="2.5" /></svg>}
                  progress={orthoConversionRate}
                  progressLabel={`${orthoConverted} verified / ${orthoLeadCount} ortho leads`}
                />
              </div>
            </SectionCard>

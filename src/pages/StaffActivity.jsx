      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', position: 'relative', zIndex: 10, width: '100%', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <div style={{ fontSize: 11.5, fontWeight: 800, color: '#475569', textTransform: 'uppercase', letterSpacing: 0.5 }}>{label}</div>
            {subtext && <div title={subtext} style={{ fontSize: 10, background: 'rgba(241,245,249,0.9)', color: '#64748b', padding: '2px 6px', borderRadius: 4, cursor: 'help', border: '1px solid #cbd5e1' }}>ℹ</div>}
          </div>
          <div style={{ width: 24, height: 24, borderRadius: '50%', background: 'rgba(255,255,255,0.8)', border: `1px solid ${color}30`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxShadow: '0 2px 6px rgba(0,0,0,0.05)' }} className="group-hover:scale-110 transition-transform">
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', position: 'relative', zIndex: 10, marginTop: 16 }}>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
            <div style={{ fontSize: 34, fontWeight: 900, color: color, lineHeight: 1, letterSpacing: '-0.02em' }}>
              {value !== undefined ? formatter(value) : <span style={{ opacity: .4, fontSize: 20 }}>—</span>}
            </div>
            <span style={{ fontSize: 10, fontWeight: 700, opacity: 0.7, textTransform: 'uppercase', letterSpacing: 1, color }}>{unitLabel}</span>
          </div>
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 18, flexWrap: 'wrap', gap: 10 }}>
        <div>
          <div style={{ fontSize: 17, fontWeight: 800, color: '#0f172a', fontFamily: "'Outfit', sans-serif" }}>{title}</div>
...
const PRESETS = [
  ...
  { label: <><Calendar size={14} className="inline-block mr-1" />{" This Month"}</>, value: 'month' },
  { label: <><Clock size={14} className="inline-block mr-1" />{" Today"}</>, value: 'today' },
  { label: <><Calendar size={14} className="inline-block mr-1" />{" Yesterday"}</>, value: 'yesterday' },
  { label: <><Search size={14} className="inline-block mr-1" />{" Custom"}</>, value: 'custom' },
];

const ROLES = [
  { label: <><Users size={14} className="inline-block mr-1" />{" All Teams"}</>, value: 'all' },
  { label: <><Briefcase size={14} className="inline-block mr-1" />{" Sales Team"}</>, value: 'sales' },
    ...
    <div style={{ minHeight: '100vh', background: '#f0f4f0', fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '24px 20px' }}>
        ...
        <div className="no-print" style={{
          ...
          {/* Row 1: Filters (Left) + Search Bar (Right) */}
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, borderBottom: '1px solid rgba(0, 0, 0, 0.06)', paddingBottom: 14 }}>
            {/* Left: Presets & Role Filters */}
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
              {/* Period Presets */}
              <div style={{ display: 'flex', gap: 3, background: '#f1f5f9', borderRadius: 12, padding: 3, border: '1px solid #cbd5e1' }}>
                {PRESETS.map(p => (
                  <button key={p.value} onClick={() => setPeriod(p.value)}
                    style={{
                      padding: '6px 12px', borderRadius: 9, border: 'none', fontSize: 12, fontWeight: 800, cursor: 'pointer',
                      background: period === p.value ? '#16a34a' : 'transparent',
                      color: period === p.value ? '#fff' : '#475569',
                      boxShadow: period === p.value ? '0 2px 6px rgba(22, 163, 74, 0.3)' : 'none',
                      transition: 'all .15s',
                    }}>{p.label}</button>
                ))}
              </div>
                ...
                {ROLES.map(r => (
                  <button key={r.value} onClick={() => setRoleFilter(r.value)}
                    style={{
                      ...
                      color: roleFilter === r.value ? '#fff' : '#475569',
                      boxShadow: roleFilter === r.value ? '0 2px 6px rgba(15, 23, 42, 0.3)' : 'none',
                      transition: 'all .15s',
                    }}>{r.label}</button>
                ))}
              </div>
              ...
              {/* Custom date range */}
              {period === 'custom' && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#fff', padding: '3px 10px', borderRadius: 10, border: '1px solid #cbd5e1' }}>
                  <input type="date" value={fromDate} onChange={e => setFromDate(e.target.value)}
                    style={{ padding: '4px 8px', borderRadius: 6, border: 'none', fontSize: 12, fontWeight: 700, outline: 'none', background: 'transparent' }} />
                  <span style={{ color: '#94a3b8', fontSize: 12, fontWeight: 800 }}>to</span>
            ...
            <div style={{ position: 'relative', width: 280, maxWidth: '100%' }}>
              <input
                ...
                style={{ padding: '7px 14px 7px 34px', borderRadius: 10, border: '1px solid #cbd5e1', fontSize: 12, width: '100%', fontWeight: 600, outline: 'none', background: '#ffffff', color: '#0f172a', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}
              />
              <Search size={14} style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', opacity: 0.6 }} />
            </div>
          </div>

          {/* Row 2: Live Metric Pills (Left) + Actions (Right) */}
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', items: 'center', justifyContent: 'space-between' }}>
            {/* Left: Quick Rate Badges */}
            <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
              <div style={{ background: 'linear-gradient(135deg, #f0fdf4, #dcfce7)', border: '1px solid #86efac', padding: '5px 12px', borderRadius: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 10, fontWeight: 800, color: '#16a34a', textTransform: 'uppercase', letterSpacing: 0.5 }}>New (1st Kit) Share:</span>
                <span style={{ fontSize: 15, fontWeight: 900, color: '#15803d', lineHeight: 1 }}>
                  {summary.totalNewDel} <span style={{ fontSize: 11, fontWeight: 700 }}>Orders <Star size={11} className="inline-block ml-0.5" /></span>
                </span>
              </div>
              <div style={{ background: 'linear-gradient(135deg, #eff6ff, #dbeafe)', border: '1px solid #93c5fd', padding: '5px 12px', borderRadius: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 10, fontWeight: 800, color: '#2563eb', textTransform: 'uppercase', letterSpacing: 0.5 }}>Total Delivered:</span>
                <span style={{ fontSize: 15, fontWeight: 900, color: '#1d4ed8', lineHeight: 1 }}>
                  {summary.totalUniqueDelivered} <span style={{ fontSize: 11, fontWeight: 700 }}>Orders</span>
                </span>
                {summary.totalUnattributed > 0 && (
                  <span title={`${summary.totalUnattributed} order(s) not linked to any Sales/Support staff`}
                    style={{ fontSize: 9.5, color: '#64748b', fontWeight: 600 }}>
                    ({summary.totalUnattributed} unlinked)
                  </span>
                )}
              </div>
              <div style={{ background: 'linear-gradient(135deg, #fef2f2, #fee2e2)', border: '1px solid #fca5a5', padding: '5px 12px', borderRadius: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 10, fontWeight: 800, color: '#dc2626', textTransform: 'uppercase', letterSpacing: 0.5 }}>RTO Impact Rate:</span>
                <span style={{ fontSize: 15, fontWeight: 900, color: '#b91c1c', lineHeight: 1 }}>
                  {summary.rtoRate}%
                </span>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginLeft: 'auto' }}>
              {lastUpdated && <span style={{ fontSize: 11, color: '#64748b', fontWeight: 700 }}>Updated {lastUpdated}</span>}
              <button onClick={fetchStaffStats} title="Refresh Live Data" style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', borderRadius: 9, padding: '7px 14px', cursor: 'pointer', color: '#334155', display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, fontWeight: 800 }}>
                <svg width={14} height={14} fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><path d="M23 4v6h-6"/><path d="M1 20v-6h6"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/></svg>
                <span>Sync</span>
              </button>
              <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: '#334155', fontWeight: 800, cursor: 'pointer', userSelect: 'none' }}>
              ...
              <button onClick={() => window.print()} title="Print Report" style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', borderRadius: 9, padding: '7px 12px', cursor: 'pointer', color: '#334155', display: 'flex', alignItems: 'center' }}>
                <svg width={14} height={14} fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24"><polyline points="6 9 6 2 18 2 18 9"/><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect x="6" y="14" width="12" height="8"/></svg>
              </button>
            </div>
          </div>
        </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 bg-slate-50">
                {ordersLoading ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: 260, color: '#94a3b8' }}>
                    <Package size={44} className="mb-2 text-slate-400" />
